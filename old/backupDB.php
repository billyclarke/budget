...
if (!@($GLOBALS["___mysqli_ston"] = mysqli_connect(DB_HOST,  DB_USER,  DB_PASS))) {
...
$mysql_server_info = ((is_null($___mysqli_res = mysqli_get_server_info($GLOBALS["___mysqli_ston"]))) ? false : $___mysqli_res);
...
if (defined('DB_NAME')) {
	...
	$db_name_list = (($___mysqli_tmp = mysqli_query($GLOBALS["___mysqli_ston"], "SHOW DATABASES")) ? $___mysqli_tmp : false);
	while (list($dbname) = mysqli_fetch_array($db_name_list)) {
...
if (isset($_REQUEST['StartBackup']) && ($_REQUEST['StartBackup'] == 'partial')) {
	...
	foreach ($ListOfDatabasesToMaybeBackUp as $dbname) {
		$tables = mysqli_query($GLOBALS["___mysqli_ston"], "SHOW TABLES FROM $dbname");
		if (is_resource($tables)) {
			echo '<table border="1"><tr><td colspan="'.ceil(mysqli_num_rows($tables) / TABLES_PER_COL).'"><b>'.$dbname.'</b></td></tr><tr><td nowrap valign="top">';
			$tablecounter = 0;
			while (list($tablename) = mysqli_fetch_array($tables)) {
				$TableStatusResult = mysqli_query($GLOBALS["___mysqli_ston"], 'SHOW TABLE STATUS LIKE "'.((isset($GLOBALS["___mysqli_ston"]) && is_object($GLOBALS["___mysqli_ston"])) ? mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $tablename) : ((trigger_error("[MySQLConverterToo] Fix the mysql_escape_string() call! This code does not work.", E_USER_ERROR)) ? "" : "")).'"');
				if ($TableStatusRow = mysqli_fetch_assoc($TableStatusResult)) {
					if (in_array(@$TableStatusRow[$TypeEngineKey], $NeverBackupDBtypes)) {
						...
						$SQLquery = 'SELECT COUNT(*) AS '.BACKTICKCHAR.'num'.BACKTICKCHAR.' FROM '.BACKTICKCHAR.((isset($GLOBALS["___mysqli_ston"]) && is_object($GLOBALS["___mysqli_ston"])) ? mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $tablename) : ((trigger_error("[MySQLConverterToo] Fix the mysql_escape_string() call! This code does not work.", E_USER_ERROR)) ? "" : "")).BACKTICKCHAR;
		...
		if (((OUTPUT_COMPRESSION_TYPE == 'gzip')  && ($zp = @gzopen($backupabsolutepath.$tempbackupfilename, 'wb'.OUTPUT_COMPRESSION_LEVEL))) ||
			...
			foreach ($SelectedTables as $dbname => $selectedtablesarray) {
				...
				foreach ($selectedtablesarray as $selectedtablename) {
					...
					set_time_limit(60);
					$result = mysqli_query($GLOBALS["___mysqli_ston"], 'CHECK TABLE '.BACKTICKCHAR.$selectedtablename.BACKTICKCHAR);
					while ($row = mysqli_fetch_assoc($result)) {
						set_time_limit(60);
						if ($row['Msg_text'] == 'OK') {

							mysqli_query($GLOBALS["___mysqli_ston"], 'OPTIMIZE TABLE '.BACKTICKCHAR.$selectedtablename.BACKTICKCHAR);
							...
							$fixresult = mysqli_query($GLOBALS["___mysqli_ston"], 'REPAIR TABLE '.BACKTICKCHAR.$selectedtablename.BACKTICKCHAR.' EXTENDED');
							$ThisCanContinue = false;
							while ($fixrow = mysqli_fetch_assoc($fixresult)) {
			...
			foreach ($SelectedTables as $dbname => $value) {
				...
				for ($t = 0; $t < count($SelectedTables[$dbname]); $t++) {
					...
					$result = mysqli_query($GLOBALS["___mysqli_ston"], 'SHOW FIELDS FROM '.BACKTICKCHAR.$SelectedTables[$dbname][$t].BACKTICKCHAR);
					while ($row = mysqli_fetch_assoc($result)) {
					...
					$uniquekeys   = array();
					$fulltextkeys = array();
					$result = mysqli_query($GLOBALS["___mysqli_ston"], 'SHOW INDEX FROM '.BACKTICKCHAR.$SelectedTables[$dbname][$t].BACKTICKCHAR);
					$INDICES = array();
					while ($row = mysqli_fetch_assoc($result)) {
					...
					$TableStatusResult = mysqli_query($GLOBALS["___mysqli_ston"], 'SHOW TABLE STATUS LIKE "'.((isset($GLOBALS["___mysqli_ston"]) && is_object($GLOBALS["___mysqli_ston"])) ? mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $SelectedTables[$dbname][$t]) : ((trigger_error("[MySQLConverterToo] Fix the mysql_escape_string() call! This code does not work.", E_USER_ERROR)) ? "" : "")).'"');
					if (!($TableStatusRow = mysqli_fetch_assoc($TableStatusResult))) {
			...
			if ($_REQUEST['StartBackup'] != 'structure') {
				...
				foreach ($SelectedTables as $dbname => $value) {
					...
					((bool)mysqli_query($GLOBALS["___mysqli_ston"], "USE $dbname"));
					for ($t = 0; $t < count($SelectedTables[$dbname]); $t++) {
						$result = mysqli_query($GLOBALS["___mysqli_ston"], 'SELECT * FROM '.$SelectedTables[$dbname][$t]);
						...
						for ($i = 0; $i < (($___mysqli_tmp = mysqli_num_fields($result)) ? $___mysqli_tmp : false); $i++) {
							$fieldnames[] = ((($___mysqli_tmp = mysqli_fetch_field_direct($result, 0)->name) && (!is_null($___mysqli_tmp))) ? $___mysqli_tmp : false);
						}
									...
									switch ($RowTypes[$dbname][$SelectedTables[$dbname][$t]][$val]) {
										...
										case 'longblob':
											...
											if ($HexBLOBs && $data_len) {
												...
												$valuevalues[] = QUOTECHAR.((isset($GLOBALS["___mysqli_ston"]) && is_object($GLOBALS["___mysqli_ston"])) ? mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $data) : ((trigger_error("[MySQLConverterToo] Fix the mysql_escape_string() call! This code does not work.", E_USER_ERROR)) ? "" : "")).QUOTECHAR;
											}
											break;
										...
										case 'decimal':
										case 'year':
											$valuevalues[] = ((isset($GLOBALS["___mysqli_ston"]) && is_object($GLOBALS["___mysqli_ston"])) ? mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $row[$key]) : ((trigger_error("[MySQLConverterToo] Fix the mysql_escape_string() call! This code does not work.", E_USER_ERROR)) ? "" : ""));
											break;
										...
										case 'timestamp':
										default:
											$valuevalues[] = QUOTECHAR.((isset($GLOBALS["___mysqli_ston"]) && is_object($GLOBALS["___mysqli_ston"])) ? mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $row[$key]) : ((trigger_error("[MySQLConverterToo] Fix the mysql_escape_string() call! This code does not work.", E_USER_ERROR)) ? "" : "")).QUOTECHAR;
											break;
									}
	...
	echo '<input type="submit" value="Backup Database"><div style="visibility: hidden;">';
	if (isset($_POST['DB_HOST'])) {
		echo '<input type="hidden" name="DB_HOST" value="'.htmlspecialchars(@$_POST['DB_HOST'], ENT_QUOTES).'">';
		echo '<input type="hidden" name="DB_USER" value="'.htmlspecialchars(@$_POST['DB_USER'], ENT_QUOTES).'">';
		echo '<input type="hidden" name="DB_PASS" value="'.htmlspecialchars(@$_POST['DB_PASS'], ENT_QUOTES).'">';
	...
	echo '</select><br>';
	echo '<input type="checkbox" name="mailto" checked="checked"></div>';
	echo '</form>';
}
